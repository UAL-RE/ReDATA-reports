...
def run(args):
    ...
    if args.outfile:
        ...
        outfile.write(
            f'id,version,size ({args.units}),title,type,is_public,published (UTC),last_modified (UTC),embargo date (UTC),embargo type,embargo options,is embargoed,report date\n')
    else:
